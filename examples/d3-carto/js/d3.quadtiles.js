...
d3.quadTiles = function(projection, zoom) {
  ...
  function visit(x1, y1, x2, y2) {
    ...
    for (var x = x1; x < x2 + Î´ / 2 && invisible; x += Î´) stream.point(x, m1);
    for (var y = m1; (y += Î´) < m2 && invisible;) stream.point(x2, y);
    for (var x = x2; x > x1 - Î´ / 2 && invisible; x -= Î´) stream.point(x, m2);
    for (var y = m2; (y -= Î´) > m1 && invisible;) stream.point(x1, y);
    if (invisible) stream.point(x1, m1);
    stream.lineEnd(), stream.polygonEnd();
    if (w <= 360 / width) {
      // TODO :)
      if (!invisible) tiles.push({type: "Polygon", coordinates: [
        d3.range(x1, x2 + Î´ / 2, Î´).map(function(x) { return [x, y1]; })
          .concat([[x2, .5 * (y1 + y2)]])
          .concat(d3.range(x2, x1 - Î´ / 2, -Î´).map(function(x) { return [x, y2]; }))
          .concat([[x1, .5 * (y1 + y2)]])
          .concat([[x1, y1]]).map(function(d) { return [d[0], mercatorÏ†(d[1])]; })
